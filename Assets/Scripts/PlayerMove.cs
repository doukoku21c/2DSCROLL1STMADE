...
public class PlayerMove : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.tag == "EndGame") // endgame 태그에 부딛힐때 
        {
            if (gameManager.stagePoint >= 130000)  // 스코어가 13만이상이면 씬 endgame1으로 이동
            {
                Time.timeScale = 1;
            ...
            else if (gameManager.stagePoint >= 110000)  // 스코어가 11만이상이면 씬 endgame2으로 이동
            {
                Time.timeScale = 1;
            ...
            else  // 스코어가 200이하이면  씬 endgame3으로 이동 
            {
                Time.timeScale = 1;
